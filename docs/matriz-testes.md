...
| TST-004 | Unitários | `python3 -m unittest discover -s tests -p 'test_*.py'` | Unitário | Local | PASSOU | 46 testes |
| TST-005 | Unitários venv | `.venv/bin/python -m unittest discover -s tests -p 'test_*.py'` | Unitário | Local | PASSOU | 46 testes via `tests/run.sh` |
...
| TST-017 | MCP parâmetros comuns | schemas aceitam `context`, `kubeconfig`, `timeout`, `output`, `verbose` e mantêm parâmetros antigos como obsoletos | Unitário | Local | PASSOU | `tests/test_mcp_server.py` |
...
| TST-032 | Scripts CRC | `scripts/preflight.sh`, `scripts/validar-contexto.sh`, `scripts/resumo-saude.sh` | Integração consultiva | CRC | PASSOU COM RESSALVA | `resumo-saude.sh` cria coleta local |
| TST-033 | MCP CRC | ferramentas solicitadas com `schema_version=1.0` e `execution_status=success` | Integração consultiva | CRC | PASSOU COM RESSALVA | payload legado preservado em `result` |
...
| TST-039 | Must-gather análise offline | índice técnico e scan de sensibilidade sem modificar `raw/` | Segurança | Local | PASSOU COM RESSALVA | 1294 indicadores potenciais de sensibilidade |
