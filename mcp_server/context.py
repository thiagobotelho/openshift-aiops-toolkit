...
def validate_cluster_context(cluster_name: str, current: str | None = None) -> dict[str, Any]:
    selected=select_cluster_context(cluster_name)
    if not selected.get('found'): return selected
    expected=validate_context(selected['cluster'].get('context','')); current_value=current or run_oc(['config','current-context'], timeout=30).stdout.strip()
