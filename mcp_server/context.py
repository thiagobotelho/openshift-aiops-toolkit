...
def current_context(timeout: int = 30, context: str | None = None, kubeconfig: str | None = None) -> dict[str, Any]:
    return run_oc(['config','current-context'], timeout=timeout, context=context, kubeconfig=kubeconfig).to_dict()
...
def validate_cluster_context(cluster_name: str, current: str | None = None, timeout: int = 30, context: str | None = None, kubeconfig: str | None = None) -> dict[str, Any]:
    selected=select_cluster_context(cluster_name)
    if not selected.get('found'): return selected
    expected=validate_context(selected['cluster'].get('context','')); current_value=current or run_oc(['config','current-context'], timeout=timeout, context=context, kubeconfig=kubeconfig).stdout.strip()
